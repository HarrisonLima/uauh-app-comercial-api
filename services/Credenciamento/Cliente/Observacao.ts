import ObservacaoModel from "../../../models/Credenciamento/Cliente/Observacao";

const ObservacaoRepository = require("../../../repositories/Credenciamento/Cliente/Observacao");

const insertObservacao = async (req: any): Promise<any> => {
  const observacao = new ObservacaoModel(req[0].cliente, req[0].observacao);
  const userId = req[1];
